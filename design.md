...
## Kinetic AI Implementation Notes

This project applies the Claude design system with these substitutes:

| Claude token | Implementation |
|---|---|
| Copernicus / Tiempos | `Cormorant Garamond` via `next/font/google` |
| StyreneB | `Inter` via `next/font/google` |
| JetBrains Mono | `JetBrains Mono` via `next/font/google` |
| CSS tokens | `src/app/globals.css` `@theme` block |
| Component classes | `.btn-primary`, `.btn-secondary`, `.feature-card`, `.callout-coral`, `.footer-dark`, `.text-input`, `.badge-pill` |
