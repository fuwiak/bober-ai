...
const nextConfig: NextConfig = {
  ...
  async redirects() {
    return [
      // Apex → www делается в middleware (с исключением verification-файлов Вебмастера/GSC).
  ...
  async headers() {
    // Next merges matching header rules; for the same key the *last* match wins.
    // Never put Cache-Control on a catch-all — it used to overwrite immutable
    // for /_next/static, /hero, fonts, etc.
    const immutable = [
    ...
    return [
      ...
      // Security only — no Cache-Control (would clobber immutable on assets).
      {
        source: "/:path*",
