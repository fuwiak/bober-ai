    ...
    <LegalPageLayout title="Политика обработки персональных данных" updatedAt={POLICY_UPDATED_AT}>
      ...
      <section>
        ...
        <ul className="mt-3 list-inside list-disc space-y-1">
          ...
          <li>
            скрипт Varioqub загружается только после нажатия «Принять» и не активируется при выборе «Без
            экспериментов».
          </li>
        </ul>

        <h3 className="mt-4 font-normal text-ink">7.3. Порядок уведомления о cookies</h3>
        <p className="mt-3">
          Скрипт «Яндекс.Метрики» загружается при посещении Сайта. Во всплывающем уведомлении пользователь может
          дополнительно согласиться на эксперименты «Varioqub» («Принять») или отказаться от них («Без
          экспериментов»). Отказ от экспериментов не отключает учёт посещаемости через «Яндекс.Метрику».
        </p>
        <p className="mt-3">
