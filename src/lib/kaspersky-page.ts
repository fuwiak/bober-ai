...
export const KASPERSKY_PAGE = {
  ...
  faqTitleRu: "Частые вопросы по Kaspersky и безопасному ИИ",
  faqRu: [
    {
      q: "Вы продаёте лицензии Kaspersky напрямую?",
      a: "Поставка идёт через официальную дистрибуцию, а наш статус — Registered Partner «Лаборатории Касперского» (B2B и B2C). Мы подбираем редакцию под задачу, считаем количество мест, готовим смету и ведём продления. Возможность реселла отдельных сервисов AIST и KIRA уточняется у партнёрского менеджера под конкретный проект.",
    },
    {
      q: "Чем это отличается от «антивируса для чат-бота»?",
      a: "Антивирус защищает рабочие места, а не логику ИИ-контура. Мы закрываем сам контур: доступ агента к системам через Secure AI Agent Gateway, разграничение прав в RAG, защиту контейнеров, где крутится модель, и разбор инцидентов в SIEM. Продукты Kaspersky отвечают за инфраструктурный слой, мы — за прикладной.",
    },
    {
      q: "Что делает Kaspersky, а что вы?",
      a: "Kaspersky даёт продукты и экспертизу: оценка модели и данных (AIST), Container Security и KIRA, KUMA и XDR, endpoint и почтовый контур. Мы внедряем и связываем это с вашим ИИ: подготовка систем, интеграции, RAG по регламентам, тикеты и автоматизация, gateway для агентов, эскалации и отчётность.",
    },
    {
      q: "Сколько стоит и с чего начинать?",
      a: "Вход — аудит контура и смета лицензий: от 150 000 ₽, 1–2 недели. Пакет Secure Private AI Cloud (приватный ИИ + российское облако или on-premise + защита Kaspersky) — от 800 000 ₽, около 28 дней. Промышленное внедрение ИИ-контура — от 500 000 ₽. Все суммы фиксируются до старта.",
    },
    {
      q: "Данные остаются в российском контуре?",
      a: "Да, если это требование проекта. Разворачиваем в Yandex Cloud, Selectel или Cloud.ru либо on-premise на вашем железе. Модель, индекс RAG и логи не выходят за периметр, доступ разграничивается по ролям, запросы логируются для аудита. Работаем по NDA, в режиме 152-ФЗ.",
    },
    {
      q: "Можно ли защитить почту от фишинга, сгенерированного нейросетью?",
      a: "Да, это отдельная линия: Kaspersky Security for Mail Server плюс наши правила и разбор инцидентов. ИИ-фишинг отличается от классического качеством текста, поэтому обучение сотрудников и автоматический разбор подозрительных писем работают лучше, чем один только фильтр.",
    },
    {
      q: "У нас нет своего SOC. Это проблема?",
      a: "Нет. Типовой сценарий — KUMA плюс ИИ-ассистент, который снимает первичный разбор: группирует события, поднимает регламент из базы знаний и готовит эскалацию. Полный штат дежурной смены для старта не нужен.",
    },
    {
      q: "Работаете с контурами 1С и терминальными серверами?",
      a: "Да. Защита серверов 1С и RDS — одна из линий: Kaspersky Endpoint Security плюс политики Kaspersky Security Center, с учётом нагрузки терминальных сессий. Это же обычно первый шаг перед подключением ИИ к данным 1С.",
    },
  ],
  attachEn: [
